...
class AIADProcessor(DataProcessor):
    def __init__(self):
        super().__init__()
        self.labels = ["bad", "good"]

    def get_examples(self, data_set, split):
        if split == "valid" or split == "dev":
            split = "validation"
        dataset = data_set
        return list(map(self.transform, dataset))
    ...
    def transform(self, example):
        text_a = example[0]
        if example[1] == "negative":#self.labels[0]:
            label_ = 0
        else:
            label_ = 1
        #guid = "{}".format(example['idx'])
        return InputExample(text_a = text_a, label=label_)
