...
parser.add_argument("--model_name_or_path", default='/nfs/users/weicheng/textCL/OpenPrompt/plm/mt5-small')
...
if os.path.exists(f"/nfs/users/weicheng/textCL/OpenPrompt/scripts/{scriptsbase}/generation_verbalizer.{scriptformat}"):
    myverbalizer = GenerationVerbalizer(tokenizer, classes=class_labels, is_rule=True).from_file(f"/nfs/users/weicheng/textCL/OpenPrompt/scripts/{scriptsbase}/generation_verbalizer.{scriptformat}", choice=args.verbalizer_id)
else:
    myverbalizer = GenerationVerbalizer(tokenizer, classes=class_labels, is_rule=False).from_file(f"/nfs/users/weicheng/textCL/OpenPrompt/scripts/{scriptsbase}/manual_verbalizer.{scriptformat}", choice=args.verbalizer_id)
