...
parser.add_argument("--gpu", type=str, default="0123")

parser.add_argument("--shot", type=int, default=32)
parser.add_argument("--k", type=int, default=4, help="number of groups for softtokens")
...
print(sys.path)
import soft_ins_vec_adaptive
import global_var
from soft_ins_vec_adaptive import SoftTemplate_ins_vec
...
mytemplate = SoftTemplate_ins_vec(model=plm, tokenizer=tokenizer, num_tokens=args.soft_token_num, initialize_from_vocab=args.init_from_vocab, k=args.k).from_file(f"/nfs/users/weicheng/textCL/OpenPrompt/scripts/{scriptsbase}/soft_template.txt", choice=args.template_id)
...
optimizer_grouped_parameters2 = [{'params': [p for name, p in prompt_model.template.vec_emb.named_parameters() if 'raw_embedding' not in name]}] # note that you have to remove the raw_embedding manually from the optimization
...
# for name, p in prompt_model.template.vec_emb.named_parameters():
#     if p.requires_grad:
#         print(name)
