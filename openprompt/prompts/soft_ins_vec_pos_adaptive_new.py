...
sys.path.append(os.path.abspath(
    '/nfs/users/weicheng/textCL/OpenPrompt/tutorial'))
...
class MLP_vec(nn.Module):
    ...
    # k is the hyper-parameter that is the number of groups of
    def __init__(self, num_soft_token, k, dim, random_range):
        super().__init__()
        self.layers = nn.Sequential(
            # nn.Flatten(),
            nn.Linear(dim, k)
        ...
        soft_embeds_group_ini = torch.FloatTensor(
            k, num_soft_token, dim).uniform_(-random_range, random_range)
        self.soft_embeds_group_param = nn.Parameter(
            soft_embeds_group_ini, requires_grad=True)

    def forward(self, x):
        '''Forward pass'''
        attention = self.layers(x)
        attention = torch.softmax(attention, 1)
        res = torch.einsum(
            "ik,kjm->ijm", [attention, self.soft_embeds_group_param])
        return res


class SoftTemplate_ins_vec_pos(Template):
    ...
    def __init__(self,
        ...
        if soft_embeds is not None:
            ...
            if self.num_tokens > 0:
                ...
                self.vec_emb = MLP_vec(
                    self.num_tokens, k, self.raw_embedding.weight.size(1), self.random_range)
                self.pos_transform = MLP_pos(
                    self.raw_embedding.weight.size(1), self.num_tokens)
                # self.pos_transform.cuda()

    def on_text_set(self):
        self.text = self.parse_text(self.text)

    # TODO this automatic generated template may not be able to process diverse data format.
    def wrap_one_example(self, example) -> List[Dict]:
        if self.text is None:
            logger.warning(
                "You didn't provide text template for softprompt. Using default template, is this intended?")
    ...
    def generate_parameters(self) -> None:
        ...
        if self.initialize_from_vocab:
            soft_embeds = self.raw_embedding.weight[:self.num_tokens].clone(
            ).detach()
        else:
        ...
        position_logit_ini = torch.FloatTensor(
            self.num_tokens+1, 1).uniform_(-self.random_range, self.random_range)  # , 0)
        
        soft_em = soft_embeds.repeat(self.num_tokens+1, 1, 1)

        self.soft_embeds = nn.Parameter(soft_em.cuda(), requires_grad=True)
            ...
            if res.dim() == 3:
                ...
                for j in range(1, self.num_tokens + 1):
                    output = output + logit[i, j] * torch.cat(
                        [torch.cat([torch.cat([soft_embeds[i, j:, :], inputs_embeds[i, :eos_position[i], :]], dim=0), soft_embeds[i, :j, :]], dim=0),
                         inputs_embeds[i, eos_position[i]:, :]], dim=0)
                res[i, :, :] = output  #
            else:
                output = logit[i, 0] * torch.roll(res[i, :], 0, 0)
                # torch.roll(res[i, :], j, 0)
                for j in range(1, self.num_tokens + 1):
                    output = output + logit[i, j] * torch.cat(
                        [torch.cat([torch.cat([soft_embeds[i, j:], inputs_embeds[i, :eos_position[i]]], dim=0), soft_embeds[i, :j]], dim=0),
                         inputs_embeds[i, eos_position[i]:]], dim=0)
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        input_batch = {key: input_batch[key]
                       for key in batch if key in forward_keys}
        ...
        self.soft_embeds = self.vec_emb(pooled_sentence)

        temperature = global_var.get_value('temperature')
        # pos_transform.cuda()
        position_emb = self.pos_transform(pooled_sentence)
        logit = torch.nn.functional.gumbel_softmax(
            position_emb, temperature, True, dim=1)
        ...
        self.soft_embed = torch.einsum('ik,kjm->ijm', [torch.transpose(logit,0,1), self.soft_embeds])
        self.soft_embeds = torch.squeeze(self.soft_embed )

        if self.num_tokens > 0:
            soft_embeds = self.soft_embeds
            inputs_embeds = self.assemble_soft_template(
                soft_embeds, inputs_embeds, 1, logit, eos_position)
        ...
        if 'attention_mask' in batch and self.num_tokens > 0:
            ...
            batch['attention_mask'] = self.assemble_soft_template(torch.ones(
                (batch_size, self.num_tokens), dtype=am.dtype, device=am.device), am, -1, logit_detach, eos_position)
        return batch
