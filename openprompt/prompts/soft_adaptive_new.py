...
class SoftTemplate3(Template):
    ...
    def generate_parameters(self) -> None:
        ...
        #     self.soft_embeds = self.soft_embeds.append(nn.Parameter(soft_embeds.cuda(), requires_grad=True))

        # self.soft_embeds = torch.as_tensor(self.soft_embeds)

        self.position_emb = nn.Parameter(
            position_logit_ini.cuda(), requires_grad=True)
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        logit = torch.nn.functional.gumbel_softmax(
            self.position_emb, temperature, True, dim=0)  # torch.softmax(self.position_emb,0)#
        ...
        if self.num_tokens > 0:
            ...
            inputs_embeds = self.assemble_soft_template(
                soft_embeds, inputs_embeds, 1, logit_bt, eos_position)
