...
class MLP_vec(nn.Module):
    ...
    def __init__(self, num_soft_token, k, dim, random_range, init): # k is the hyper-parameter that is the number of groups of
        super().__init__()
        ...
        if init==None:
            soft_embeds_group_ini = torch.FloatTensor(k, num_soft_token, dim).uniform_(-random_range, random_range)
            self.soft_embeds_group_param = nn.Parameter(soft_embeds_group_ini, requires_grad=True)
        else:
            self.soft_embeds_group_param = nn.Parameter(init, requires_grad=True)
    ...
    def forward(self, x):
        ...
        res = torch.einsum("ik,kjm->ijm", [attention, self.soft_embeds_group_param])
        return res

class SoftTemplate_ins_vec(Template):
        ...
        if soft_embeds is not None:
            ...
            if self.num_tokens>0:
                ...
                if self.initialize_from_vocab:
                    soft_embeds = self.raw_embedding.weight[:self.num_tokens * self.k].clone().detach()
                    soft_embeds = torch.stack(torch.split(soft_embeds, self.num_tokens, dim=0))
                    self.vec_emb = MLP_vec(self.num_tokens, k, self.raw_embedding.weight.size(1), self.random_range,
                                           init=soft_embeds)
                else:
                    self.vec_emb = MLP_vec(self.num_tokens, k, self.raw_embedding.weight.size(1), self.random_range,init=None)
    ...
    def assemble_soft_template(self, soft_embeds, inputs_embeds, dim):
        res = []
        if len(self.idx) == 1:
            ...
            for i in range(len(self.idx)):
                if res.dim() == 3:
                    res[i, :, :] = torch.roll(res[i,:,:], self.idx[i].item(), 0)
                else:
                    res[i, :] = torch.roll(res[i, :], self.idx[i].item(), 0)
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        if 'attention_mask' in batch and self.num_tokens > 0:
            ...
            batch['attention_mask'] = torch.cat(
                [torch.ones((batch_size, self.num_tokens), dtype=am.dtype, device=am.device), am], dim=-1)
        return batch
