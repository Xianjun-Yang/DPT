...
class MLP_vec(nn.Module):
    ...
    def __init__(self, num_soft_token, k, dim, random_range): # k is the hyper-parameter that is the number of groups of
        super().__init__()
...
class SoftTemplate_pos_ins_vec(Template):
    ...
    def __init__(self,
        ...
        if soft_embeds is not None:
            ...
            if self.num_tokens>0:
                ...
                position_logit_ini = torch.FloatTensor(self.num_tokens + 1, 1).uniform_(-self.random_range,
                                                                                        self.random_range)
                self.position_emb = nn.Parameter(position_logit_ini, requires_grad=True)
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        # print("temperature: \n", temperature)
        logit = torch.nn.functional.gumbel_softmax(self.position_emb, temperature, True, dim=0)
