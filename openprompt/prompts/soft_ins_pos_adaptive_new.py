...
class MLP_pos(nn.Module):
    ...
    def __init__(self, dim_size, num_soft_token):
        ...
        self.layers = nn.Sequential(
            ...
            # nn.Linear(dim_size, 2048),#num_soft_token + 1),
            # nn.ReLU(),
            # nn.Linear(2048, 1024),
            # nn.ReLU(),
            # nn.Linear(1024, 100),
            # nn.ReLU(),
            # nn.Linear(100, num_soft_token + 1),
            # nn.ReLU(),
        )
...
class SoftTemplate_ins_pos(Template):
    ...
    def generate_parameters(self) -> None:
        ...
        #soft_em = torch.FloatTensor(self.num_tokens+1, self.num_tokens, self.raw_embedding.weight.size(
        #     1)).uniform_(-self.random_range, self.random_range)

        self.soft_embeds = nn.Parameter(soft_em.cuda(), requires_grad=True).to('cuda')
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        soft_embeds = torch.einsum('ik,kjm->ijm', [logit, self.soft_embeds])
